...
export default function TrialExpiredBanner({ daysLeft, isExpired, trialEndsAt, status }: TrialExpiredBannerProps) {
  if (!isExpired && (daysLeft === null || daysLeft === undefined || daysLeft > 0)) {
    return null;
  }

  const isPaidPlan = status && !["trial", "no_subscription"].includes(status);
